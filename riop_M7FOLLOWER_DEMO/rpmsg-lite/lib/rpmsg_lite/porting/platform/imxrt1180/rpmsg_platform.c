...
int32_t platform_init_interrupt(uint32_t vector_id, void *isr_data)
{
    if (platform_lock != ((void *)0))
    {
        ...
        {
#if defined(FSL_FEATURE_MU_SIDE_A)
            MU_EnableInterrupts(MU1_MUA, MU_GI_INTR(1UL << vector_id));
#elif defined(FSL_FEATURE_MU_SIDE_B)
            MU_EnableInterrupts(MU1_MUB, MU_GI_INTR(1UL << vector_id));
#endif
        }
        isr_counter++;
...
int32_t platform_deinit_interrupt(uint32_t vector_id)
{
    if (platform_lock != ((void *)0))
    {
        ...
        {
#if defined(FSL_FEATURE_MU_SIDE_A)
            MU_DisableInterrupts(MU1_MUA, MU_GI_INTR(1UL << vector_id));
#elif defined(FSL_FEATURE_MU_SIDE_B)
            MU_DisableInterrupts(MU1_MUB, MU_GI_INTR(1UL << vector_id));
#endif
        }
...
#if defined(FSL_FEATURE_MU_SIDE_A)
    (void)MU_TriggerInterrupts(MU1_MUA, MU_GI_INTR(1UL << (RL_GET_Q_ID(vector_id))));
#elif defined(FSL_FEATURE_MU_SIDE_B)
    (void)MU_TriggerInterrupts(MU1_MUB, MU_GI_INTR(1UL << (RL_GET_Q_ID(vector_id))));
#endif
#endif
    env_unlock_mutex(platform_lock);
}
